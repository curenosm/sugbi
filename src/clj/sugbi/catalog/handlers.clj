...
(defn search-books
  ...
  (if-let [criteria (get-in request [:parameters :query :q])]
    (response/ok
     (catalog.core/enriched-search-books-by-title criteria catalog.core/available-fields))
    (response/ok
     (catalog.core/get-books catalog.core/available-fields))))


(defn- image-type
  [temp-file]
  (let [image-type (->> temp-file :filename
                        (re-find #".*\.(jpg|png)") second)]
    image-type))

(defn insert-book!
  [request]
  (let [{:keys [isbn title file] :as _book-info} (get-in request [:parameters :multipart])
        is-librarian?    (get-in request [:session :is-librarian?])]
    (def the-file file)
    (cond (not is-librarian?)      (response/forbidden
                                     {:message "Operation restricted to librarians"})
     (nil? (image-type file))      (response/unsupported-media-type
                                     {:message "Server just admits JPG or PNG files"})
     :else                         (response/ok
                                    (select-keys
                                     (catalog.db/insert-book!
                                       {:isbn  isbn
                                        :title title
...
(defn get-book-cover
  ...
  (let [isbn       (get-in request [:parameters :path :isbn])
    ...
    (if-let [{image-data :cover_image_data
              image-type :cover_image_type} (catalog.db/get-book-cover
                                              {:isbn isbn
                                               :type media-type})]
      (-> image-data byte-streams/to-input-stream response/ok
